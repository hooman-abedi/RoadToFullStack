...
// Are all trainers verified?
const allVerified = trainersDB.every(trainer => trainer.isVerified === true)
console.log(allVerified)
// Are all published packages under $300?
const lessThanThreeHundred = packagesDB
    .filter(pkg => pkg.isPublished)
    .every(pkg => pkg.price < 300)

console.log(lessThanThreeHundred)

// Do all trainers have more than 50 students?
const allEstablished = trainersDB.every(trainer => trainer.totalStudents > 50)
console.log(allEstablished) // true — lowest is Carlos with 95
// every, stops as soon as it finds one failure. If the first item fails, it returns false immediately.
